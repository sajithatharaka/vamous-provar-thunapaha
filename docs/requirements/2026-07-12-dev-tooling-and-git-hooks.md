...
- **`db:push` fixed**: `supabase db push --project-ref $SUPABASE_PROJECT_ID` — no `--token` flag; the Supabase CLI reads `SUPABASE_ACCESS_TOKEN` from the environment automatically, and `supabase` being a local devDependency means `npx` is no longer needed.
